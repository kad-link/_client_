...
function Home() {
  ...
  try {
    ...
    const response = await fetch(`https://server-beryl-phi.vercel.app/api/recordings/${recordingId}`, {
      method: 'DELETE',
      headers: {
  ...
  const fetchRecordings = async()=>{
    ...
    try{
      ...
      const response = await fetch(`https://server-beryl-phi.vercel.app/api/recordings/${currentUser.uid}`,{
        method:"GET",
        headers: {
          'Authorization': `Bearer ${currentUser?.accessToken || ''}`,
        }
  ...
  const sendAudioToBackend = async (audioBlob, speaker, duration,UserId) => {
    try {
      ...
      const response = await fetch('https://server-beryl-phi.vercel.app/api/transcribe', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `Bearer ${currentUser?.accessToken || ''}`,
